...
import prqlPlugin from './index'
import { prql } from './utils'
import './types'
...
describe('prqlPlugin', () => {
  ...
  it('should be able to disable signature', () => {
    const compileOptions = new prqljs.CompileOptions()
    compileOptions.signature_comment = false

    const plugin = prqlPlugin({
      compileOptions,
    })
    const result = plugin.transform('prql`from a`', 'some-id')

    expect(result).not.toBeNull()
    expect(result.code).not.toContain('Generated by PRQL compiler')
  })
